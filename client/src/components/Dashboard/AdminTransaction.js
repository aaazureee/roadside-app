...
class AdminTransaction extends Component {
  ...
  async componentDidMount() {
    const { data: result } = await api.get('/admin/service-payments')
    if (result.success) {
  ...
  render() {
    ...
    const options = {
      ...
      renderExpandableRow: (rowData, rowMeta) => {
        ...
        const { address, description, vehicle } = rowData[4]
    ...
    return (
      <MuiThemeProvider theme={this.getMuiTheme()}>
        <MUIDataTable
          title={'System Service Payment History'}
          data={transactions}
          columns={columns}
          options={options}
        />
...
export default withStyles(styles)(AdminTransaction)
